...
export async function updateSetById(id: number, value: string): Promise<Set | undefined> {
    const values = [id, value];
    const sqlQuery = 'UPDATE sets SET name = $2 WHERE id = $1 RETURNING *;';
    const result = await pool.query(sqlQuery, values);
    return result.rows[0];
}
export async function deleteSetbyId(id: number): Promise<Set | undefined> {
    const sqlQuery = 'DELETE FROM sets WHERE id = $1 RETURNING *;';
    const result = await pool.query(sqlQuery, [id]);
    return result.rows[0];
}
